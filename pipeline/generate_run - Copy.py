...
def _load_persona(persona: str) -> str:
    """Load persona .md file. Returns content or empty string."""
    path = os.path.join(PERSONAS_DIR, f"{persona}.md")
    if not os.path.exists(path):
        fallback = os.path.join(PERSONAS_DIR, f"{DEFAULT_PERSONA}.md")
        if os.path.exists(fallback):
            print(f"   [persona] '{persona}.md' not found -- using {DEFAULT_PERSONA}")
...
def _retrieve_context(topic: str, item_id: str, category: str,
    ...
    try:
        ...
        try:
                ...
                if research_chunks:
                    ...
                    if resp.status_code == 200:
                        ...
                        def _coverage_sample(chunks, focus_vec, max_n, min_n, threshold, trigger_w):
                            ...
                            scored = []
                            for chunk in chunks:
                                # chunk tuple: (score, trust, text, url, id, src, payload)
                                ...
                                scored.append((combined, dimension, chunk))

                            scored.sort(key=lambda x: x[0], reverse=True)

                            selected    = {}
                            fallback    = []
                            for combined, dimension, chunk in scored:
                                if dimension not in selected:
                                    selected[dimension] = chunk
                            ...
                            # Guarantee minimum regardless of score
                            if len(result) < min_n:
            ...
            # Coverage sampling fallback -- no focus vec available without embed
            _fb_seen_dims = {}
            _fb_p_final   = []
            for chunk in sorted(_fb_persona, key=lambda x: x[0], reverse=True):
                payload = chunk[6] if len(chunk) > 6 else {}
                dim     = payload.get("dimension", "unknown")
                if dim not in _fb_seen_dims:
                    _fb_seen_dims[dim] = True
                    _fb_p_final.append(chunk)
                if len(_fb_p_final) >= PERSONA_MAX:
                    break
            if len(_fb_p_final) < PERSONA_MIN:
                for chunk in sorted(_fb_persona, key=lambda x: x[0], reverse=True):
                    if chunk not in _fb_p_final:
                        _fb_p_final.append(chunk)
                    if len(_fb_p_final) >= PERSONA_MIN:
                        break
...
def _build_prompt(topic: str, persona_text: str, section: str,
    ...
    # Use .md file if available, otherwise fall back to Qdrant chunks
    # context_persona is passed by reference so we clear it if used here
    _effective_persona = persona_text or context_persona
    if _effective_persona and not persona_text:
        # Qdrant chunks used as persona -- clear from context to avoid duplication
        context_persona = ""
    persona_block = ""
    if _effective_persona:
        persona_block = f"""
=== YOUR PERSONA ===
Write this article as the person described below.
Internalize this persona completely -- voice, opinions, style, worldview.
Do NOT write generically. Do NOT soften opinions. Do NOT hedge.
IMPORTANT: Persona defines HOW you write -- not WHAT you write about.
- Voice, tone, and directness come from the persona
- Do NOT insert hardware specs, personal setup, or biography unless directly
  relevant to the article topic (e.g. article is explicitly about that hardware)
- Persona opinions about other games/products = stylistic context only --
  do not reproduce them as standalone paragraphs or verbatim quotes
{_effective_persona}
=== END PERSONA ===
"""
    ...
    if context_research or context_persona or context_evergreen:
        ...
        if context_persona:
            author_voice_block = (
                "=== AUTHOR VOICE ===\n"
                "These are examples of how this author actually writes -- voice in action.\n"
                "Write with the same register, directness, and rhythm.\n"
                "Do NOT copy phrases verbatim. Do NOT reproduce as standalone paragraphs.\n"
                "Let the tone and attitude bleed into every sentence of the article.\n\n"
...
1. DENSITY: Every sentence must contribute a fact from research or a concrete evaluation.
   If a sentence does neither -- delete it. No filler, no throat-clearing, no transitions
   that carry no information. If you run out of facts -- stop writing. A shorter article
   built on facts beats a longer article padded with opinion fog.
...
def _ensure_schema_and_focus(item: dict) -> None:
    ...
    try:
        ...
        from menus.queue import _focus_picker, _save_focus
...
def run_generate(item_ids: list = None, api_provider: str = "", night_run: bool = False) -> int:
    ...
    for idx, item in enumerate(items, 1):
        ...
        _rq_press   = sum(1 for c in (chunk_meta or []) if c.get("trust", "unknown") in ("press", "trusted"))
        _rq_issues  = []
        ...
        if upgrade_url and upgrade_mode:
            ...
            if not article_focus and not item.get("night_run", False):
                try:
